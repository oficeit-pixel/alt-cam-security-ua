...
def yugtorg_relevant_product(group: str, name: str) -> bool:
    ...
    required = {
        ...
        "batteries": r"акумулятор|батаре|елемент\s+живлен|battery|\bакб\b",
        "battery_accessories": r"bms|баланс|клем|тримач|корпус|кабель|роз['’]?єм|перехід|тестер|заряд",
        "power_adapters": r"блок\s+живлен|адаптер|перетворювач|dc|зарядн",
        "electrical": r"автомат|реле|контактор|запобіж|пзв|узо|din|щит|клем|розет|вимикач|електр",
        "tools": r"викрутк|плоскогуб|бокоріз|стрипер|кримпер|обтиск|мультиметр|тестер|дрил|перфоратор|бур|коронк|паяль|рулетк|монтажн\w*\s+ніж|набір\s+ключ",
        "installation_accessories": r"монтажн|короб|гофр|кабель[- ]?канал|стяжк|дюбел|кріпл|хомут|ізолент|термоусад|конектор|роз['’]?єм|клем|bnc|dc",
    }
