...
function renderProducts(){
  ...
  let list=products.filter(item=>(!category.value||item.category===category.value)&&(!brand.value||item.brand===brand.value)&&(!query||[item.name,item.model,item.brand,item.sku,item.subcategory].join(' ').toLowerCase().includes(query)));
...
[search,category,brand,sort].forEach(el=>el.addEventListener(el===search?'input':'change',renderProducts));
document.querySelector('#filter-reset').addEventListener('click',()=>{search.value='';category.value='';brand.value='';sort.value='featured';renderProducts()});
document.querySelectorAll('[data-category]').forEach(button=>button.addEventListener('click',()=>{track('category_view',{category:button.dataset.category});if(button.dataset.category==='Послуги'){document.querySelector('#services').scrollIntoView()}else{category.value=button.dataset.category;renderProducts();document.querySelector('#products').scrollIntoView()}}));
